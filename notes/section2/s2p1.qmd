...
[{{< fa lightbulb >}} Predict]{.mode} --- say the answer out loud before you run it.
...
[{{< fa lightbulb >}} Predict]{.mode} --- one of these two lines warns you. Which one, and why?
...
On this page the file is already next to the notes, as `data/Cars.dat`. **Run this block first** — everything after it uses `Cars`.
...
Incoming graduate students used to take a [programming proficiency test](data/proficiency.pdf) before the semester started. We no longer give it, but the two questions are still a good warm-up. The email below is about question 2.
...
[{{< fa lightbulb >}} Predict]{.mode} --- what does the second line give? This is why the comment above says to spell `T` and `F` out.
...
This is a [Matryoshka doll]{.more note="A Russian nesting doll: open it and there is a smaller doll inside, and another inside that one."}!
...
[{{< fa lightbulb >}} Predict]{.mode} --- a factor that looks like numbers. What does the second line give?
